...
void  Headmaster::graduateStudent(Professor* p_professor, std::shared_ptr<Student> p_student, std::shared_ptr<Course> p_course)
{
	...
	if (validateForm(form))
	{
		if (p_student->getNumberOfClassAttended(p_course) >= p_course->getNumberOfClassToGraduate())
		{
			...
			p_student->removeCourse(p_course.get());
...
void Professor::teach()
{
	if (_currentCourse)
	{
		if (_currentCourse->getClassRoom()->getOccupantSize() > 1) // one person is the professor
		{
			std::cout << "Professor: " << getName() << " is teaching course: " << _currentCourse->getName() << "for " << _currentCourse->getClassRoom()->getOccupantSize() - 1 << " student(s)" << std::endl;
			_currentCourse->teachStudent();
		}
		else 
		{
			std::cout << "Professor: " << getName() << " has no student to teach" << std::endl;
		}
	}
	else 
	{
...
bool Headmaster::assignClassroomToCourse(std::shared_ptr<Course> p_course, std::shared_ptr<Classroom> p_classroom)
{
	...
	{
		p_classroom->assignCourse(p_course.get());
		return true;
	}
	return false;
}
...
void Student::removeCourse(Course* p_course)
{
...
void Professor::update(const Event &event) 
{
	...
	else if (event == Event::StartClass)
	{
		teach();
	}
}
...
void Student::update(const Event& event) 
{
	if (event == Event::RingBell)
	{
		...
		{
			if (currentRoomIdx == _subscribedCourse.size())
				currentRoomIdx = 0;
			auto courseIt = _subscribedCourse.begin();
			std::advance(courseIt, currentRoomIdx++);
			if (courseIt->first)
			{
				attendClass(courseIt->first->getClassRoom());
...
void Professor::graduateAllStudent(std::vector<std::shared_ptr<Student> > p_students)
{
	if (!_currentCourse)
	{
		std::cout << RED << "Professor: " << getName() << " has no course to teach" << BLACK << std::endl;
		return;
	}
	for (auto& student : p_students)
	{
		gradeStudent(student, _currentCourse);
	}
}
